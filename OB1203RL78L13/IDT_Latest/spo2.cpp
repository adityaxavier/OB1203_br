...
extern Timer t1;
extern Serial pc;
//#define PRINT_RAW //print raw, filtered data (for Nohayo algorithm--includes two dummy variables for current)
...
void SPO2::kalman(uint32_t *kalman_array, uint8_t *kalman_length, uint8_t *kalman_ptr, uint32_t *data_array, uint8_t *data_array_length, uint8_t *data_ptr, uint32_t new_data, volatile bool *reset_kalman, uint32_t *kalman_avg)
{
    ...
    for (int n= 0; n<MAX_KALMAN_LENGTH; n++) {
        pc.printf("%04d ",kalman_array[n]);
    }
    pc.printf("\r\ndat ");
    for (int n= 0; n<NUM_HR_AVGS; n++) {
        pc.printf("%04d ",data_array[n]);
    }    
    static uint8_t outlier_cnt =0;
    static uint8_t alg_fail_cnt = 0;
    uint32_t data_std;
//    pc.printf("\r\nk_length = %d, k_ptr = %d, d_length = %d, d_ptr = %d, new_data = %d, reset_kalman = %d, kalman_avg = %d\r\n",*kalman_length,*kalman_ptr,*data_array_length,*data_ptr,new_data,*reset_kalman,*kalman_avg);
    if (*kalman_avg == 0) *reset_kalman = 1; //reset if the previous average was invalid
    if(*reset_kalman) {//re-initialize
        pc.printf("resetting Kalman\r\n");
        *kalman_ptr = 0;
        *kalman_length = 0;
        alg_fail_cnt = 0;
        outlier_cnt = 0;
        *reset_kalman = 0;
        ...
        if(outlier_cnt >= OUTLIER_DATA_TOLERANCE) {//too many outliers: next time reset the filters
            ...
            */
            pc.printf("reset: too many outliers\r\n");
            *reset_kalman = 1;
        }
    }
}
...
#ifdef PRINT_RAW
    pc.printf("%d, %d, %d, %d, %d\r\n",t1.read_us(),dc_data[IR][data_ptr],dc_data[RED][data_ptr],1,1);
#endif
    buffer_index++;
